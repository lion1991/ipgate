...
| `auth` | `keys`（Ed25519 验签）/ `token`（HMAC 会话令牌）/ `challenge`（单次 nonce）/ `pairing`（配对码，文件跨进程） |
| `api` | axum 路由 + `AppState` + `serve`（over rustls）+ `AuthDevice` Bearer 抽取器 + 错误→HTTP 映射 |
| `main` | CLI：`run`（守护进程）/ `pair` / `print-ruleset` / `status` / `uninstall` |

## CLI

```sh
ipgate-agent --config <path> pair            # 生成一次性配对码 + 打印 SPKI 指纹（供新设备入网）
...
```sh
cargo test                  # 31 项：含完整 配对→挑战→验签→放行→撤销 端到端流程
...
- [ ] 审计日志 / 限速锁定（ADR 0003 加硬项）；实时同步 WebSocket
