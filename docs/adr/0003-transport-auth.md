...
状态：已定案；**传输 / 服务端身份 / 会话三项被 [ADR 0007](0007-noise-over-ssh.md) 取代**（TLS→Noise、强制 SSH 载体、删 Bearer 令牌）。设备身份 / 吊销 / 不自锁思路仍沿用。
日期：2026-06-23
关联：ADR 0001（整体架构）、ADR 0002（nftables 落地方式，待定）、ADR 0007（取代本 ADR 传输层）
