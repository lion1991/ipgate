...
**密钥**：每台设备首次运行生成一对 Ed25519 密钥。私钥存于
- 移动端：iOS Keychain / Android Keystore；
- 桌面端：系统密钥库（或 Stronghold / 加密文件）。
私钥**永不离开设备**。
...
- **限速 + 锁定**：按源 IP 限速；`auth/verify` 连续失败 N 次锁定该源（fail2ban 式）；全局并发上限。
- **超时**：读/写/空闲超时，防 slow-loris。
- **未配对时 `/v1/pair` 默认关闭**，仅 `ipgate-agent pair` 开启配对窗口期内放行。
- 防御纵深（**后续可选，不在 v1**）：SPA / 端口敲门隐藏端口；WireGuard 模式。
...
| 威胁 | 对策 |
|------|------|
| 公网端口被扫/爆破 | 纯密钥鉴权(无口令可爆)、限速+锁定、预鉴权面最小 |
| 中间人 | TLS 1.3 + TOFU 指纹固定；指纹变更硬失败 |
| 重放签名 | nonce 单次短时 + 挑战绑定服务端指纹 |
| 客户端/设备失窃 | 私钥存安全区;按设备吊销;令牌短 TTL |
