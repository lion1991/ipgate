...
| 主机角色 | **全主机 default-drop**：ipgate 接管 `input` hook，策略 drop；放行 established/loopback/基础设施 ICMP(不含 ping)/管理端口/公开端口/名单源。**ICMP echo(ping) 只放给名单源 IP**——未放行的 IP ping 不通、也访问不到任何东西 |
...
table inet ipgate {
    ...
    chain input {
        ...
        # 基础设施 ICMPv6：不放则 IPv6 直接瘫(邻居发现/RA/PMTUD)。**不含 echo(ping)**——
        # ping 视作探测/访问面,只放给名单源 IP(见末尾 saddr 规则),未放行 IP ping 不通。
        ...
        # 基础设施 ICMPv4：仅 PMTUD/错误类(防黑洞);echo(ping)同样不放,只走名单。
...
- ~~公开端口 ICMP echo 是否默认放行（隐蔽性 vs 可运维性）。~~ **已定（2026-06-24）：echo(ping) 默认不放，
  只放给名单源 IP**——隐蔽性优先,未放行 IP ping 不通;PMTUD 错误类仍放行以防黑洞。
  若日后需对全网开放 ping(如外部监控探活),再加 `RulesetConfig.public_icmp_echo` 开关。
